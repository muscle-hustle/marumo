...
│   ├── 05_deployment.md
│   ├── 06_mediapipe-integration.md
│   ├── 07_mediapipe-migration-guide.md
...
- 強度調整スライダー（モザイクとぼかしの場合のみ表示）
  - 強度範囲: 1-5（デフォルト: 3）
  - リアルタイムプレビュー（強度変更時に自動的に再加工）
  - モザイク: 強度が低いほど細かいモザイク（元の顔が見えやすい）
  - ぼかし: 強度が高いほど強くぼかす
- スタンプ選択時は強度調整は非表示（スタンプには強度設定が不要）
...
2. 各グリッドセルの平均色を計算
3. セル全体を平均色で塗りつぶし
4. 強度（intensity: 1-5、デフォルト: 3）に応じてグリッドサイズを調整
   - グリッドサイズ = 顔領域の最小辺 / (15 - intensity * 1.5)
   - 強度が低いほどグリッドが小さくなり、モザイクが細かくなる（元の顔が見えやすい）
   - 強度が高いほどグリッドが大きくなり、モザイクが粗くなる（元の顔が見えにくい）

#### ぼかし処理
1. 各顔領域に対してガウシアンブラーを適用
2. Canvas APIの`filter`プロパティを使用（互換性確認済み）
3. 強度（intensity: 1-5、デフォルト: 3）に応じてブラー半径を調整
   - ブラー半径 = 2 + intensity * 1.5px
   - 強度1: ブラー半径 = 3.5px（弱いぼかし）
   - 強度5: ブラー半径 = 9.5px（強いぼかし）
   - 強度が高いほどブラー半径が大きくなり、より強くぼかされる
   - 強度が低いほどブラー半径が小さくなり、より弱くぼかされる
