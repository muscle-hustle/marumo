...
import type { DetectionMode, ManualModeType, ProcessingOption, ProcessingType } from './types'
      ...
      <main className="mx-auto flex w-full max-w-6xl flex-col gap-8 px-6 py-12 sm:px-8 lg:px-12">
        <section className="grid gap-6 lg:grid-cols-[minmax(0,2fr)_minmax(0,3fr)]">
          <div className="glass-panel p-6 space-y-4">
            ...
            <ImageUploader selectedFileName={selectedFileName} onSelect={(file) => setSelectedFileName(file.name)} />
            <p className="text-xs text-white/60">PNG / JPEG / WebP、10MBまで対応。STEP2でCanvas描画が連動します。</p>
          </div>

          <div className="glass-panel p-6 space-y-4">
            <h2 className="text-lg font-display font-semibold">2. プレビュー</h2>
            <ImageCanvas status={selectedFileName ? 'loading' : 'idle'} caption={selectedFileName ? `${selectedFileName} を読み込み待機中` : undefined} />
          </div>
        </section>
